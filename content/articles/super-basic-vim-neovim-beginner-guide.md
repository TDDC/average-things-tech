...
My recommendation:
    ...
    Scoop       best for terminal/dev-tool users
    Winget      simplest built-in Windows option
    Chocolatey  good if you already use Chocolatey
    Manual      useful if package managers are not allowed

For Average Things Tech beginners, I recommend:
...
For Linux or macOS, the Vim config file is usually:
...
The most important command in this entire guide is:

    :q

That means quit.

But before typing `:q`, press:
...
To delete a full line:

    dd

Be careful. `dd` deletes the entire line.

If you delete something by mistake, press:
...
Use this to save and quit:

    :wq

Or use this to quit without saving:
